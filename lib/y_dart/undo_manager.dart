...
final class YUndoManager implements ffi.Finalizable {
  ...
  void undo() {
    gen.yundo_manager_undo(_undoManager);
  }

  void redo() {
    gen.yundo_manager_redo(_undoManager);
  }

  void stopCapturing() {
    // Should it actually be called 'commit' or 'freeze'
    gen.yundo_manager_stop(_undoManager);
  }
  ...
  void removeOrigin(YOrigin origin) {
    ...
    final originPtr = malloc<ffi.Uint8>(originBytes.length);
    originPtr.asTypedList(originBytes.length).setAll(0, originBytes);
    gen.yundo_manager_remove_origin(
      _undoManager,
      originBytes.length,
      originPtr.cast<ffi.Char>(),
    );
  ...
  void addScope(YType type) =>
      gen.yundo_manager_add_scope(_undoManager, type._branch);

  void clear() => gen.yundo_manager_clear(_undoManager);

  int get undoStackLen => gen.yundo_manager_undo_stack_len(_undoManager);

  int get redoStackLen => gen.yundo_manager_redo_stack_len(_undoManager);
}
